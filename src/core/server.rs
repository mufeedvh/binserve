use actix_web::{
    ...
    http::{
        KeepAlive,
        header::{
            ETAG, CACHE_CONTROL, SERVER, IF_MODIFIED_SINCE, IF_NONE_MATCH, LAST_MODIFIED,
...
use crate::cli::messages::{Type as MsgType, push_message};
    ...
    match ROUTEMAP.get(req.path()) {
        Some(handler) => {
            ...
            if handler.r#type == Type::File {
                ...
                return Ok(
                    NamedFile::open(handler.response.path.to_owned())?
                        .prefer_utf8(true)
                        .use_etag(true)
    ...
    let mut http_server = HttpServer::new(move || {
        let mut app_instance = App::new()
            ...
            .wrap({
                ...
                headers_middleware = headers_middleware.add((SERVER, format!("binserve/{}", VERSION)));
            ...
            .wrap({
                ...
                let mut middleware = Condition::new(
                    config_state.server.tls.enable, RedirectHttps::default()
                );
                ...
                if tls_address.len() > 1 { tls_port = tls_address[1] }

                if tls_port != "443" {
                    middleware = Condition::new(
                        config_state.server.tls.enable,
                        RedirectHttps::default().to_port(
                            tls_port.parse::<u16>().unwrap()
                        )
                    );
                }
        ...
        if !static_served_from.is_empty() && static_directory != &PathBuf::new() {
            app_instance = app_instance.service({
                let mut static_file_service = actix_files::Files::new(
                    static_served_from,
                    static_directory
                )
